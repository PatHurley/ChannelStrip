...
def  main():
  ...
  installer = "\installer\ChannelStrip Installer.exe"
   
  if demo:
    installer = "\installer\ChannelStrip Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\ChannelStrip manual.pdf" 
  ]
  ...
  ZIPNAME = "ChannelStrip-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "ChannelStrip-v" + FULLVERSIONSTR + "-win-demo.zip"
